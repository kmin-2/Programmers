...
public class level1_08 {
	...
	가장 많은 종류의 폰켓몬을 선택하는 방법이 여러 가지인 경우에도, 선택할 수 있는 폰켓몬 종류 개수의 최댓값 하나만 return 하면 됩니다.*/
	 ...
	 public static int solution(int[] nums) {
        ...
        Arrays.sort(nums);
        ArrayList<Integer> mons_distinct = new ArrayList<Integer>(); //중복제거한 몬스터리스트
        mons_distinct.add(nums[0]);
        
        int num = 0;
        for(int i = 1; i<nums.length; i++){ 
            if(mons_distinct.get(num) != nums[i]){
                num ++;
                mons_distinct.add(nums[i]);
            } 
        }
        
        int result = nums.length / 2; 
        if(mons_distinct.size() < nums.length / 2){
            result = mons_distinct.size();
        }
